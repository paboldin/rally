...
class ServerGeneratorExt(context.Context,
    ...
    """Extended context class for adding temporary servers for benchmarks.

    Boot servers in each of the tenants using affinity group if requested.
    By default the amount of servers matches hypervisors count. Floating IPs
    are associated to each of the server (floating_ips=each), only first one
    (floating_ips=once) or none (floating_ips=none).
    """
    RESOURCE_NAME_FORMAT = "rally_ctx_servers_ext_XXXXXXXX_XXXXXXXX"
    ...
    def _boot_one_server_with_ips(self, **kwargs):
        vm_scenario, server, fip = self._boot_server_for_user(**kwargs)
    ...
    def _boot_tenant_servers(self, user, tenant):
        ...
        for i in range(servers_per_tenant):
            ...
            server_with_ips = self._boot_one_server_with_ips(
                user=user,
    ...
    @logging.log_task_wrapper(LOG.info, _("Enter context: `ServersExt`"))
    def setup(self):
    ...
    @logging.log_task_wrapper(LOG.info, _("Exit context: `ServersExt`"))
    def cleanup(self):
        for user, tenant_id in rutils.iterate_per_tenants(
                self.context["users"]):
